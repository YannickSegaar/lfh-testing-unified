 ...
 *
 * Imported by the in-chat browser self-service widget.
 *
...
const RESOURCE_ICONS = {
  ...
  tours: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
const RESOURCES = [
  ...
  },
  {
    id: 'tours',
    title: 'Explore Tours',
    description: 'Compare all tour packages with pricing',
    icon: 'tours',
    type: 'action',
    handler: 'open_tours',
  },
  {
    ...
    description: 'Bell 2 vs Ripley Creek side by side',
    icon: 'lodges',
    type: 'action',
    handler: 'open_lodges',
  },
...
.lfhbss-header-bar {
  ...
  padding: 12px 20px;
  background: ${LFH_COLORS.textPrimary};
  flex-shrink: 0;
}
.lfhbss-header-left {
  display: flex;
...
.lfhbss-video-thumb::after {
  ...
  background: url('${LFH_ASSETS.videoMask}') center/cover no-repeat;
  opacity: 0.6;
  pointer-events: none;
}
.lfhbss-video-play {
...
// ============================================================================
// CHEVRON SVG (shared by FAQ)
// ============================================================================

const CHEVRON_SVG = `<svg class="lfhbss-faq-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>`;

const ARROW_SVG = `<svg class="lfhbss-resource-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>`;

const BACK_ARROW = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>`;

// ============================================================================
// MAIN EXPORT: openBrowserSelfServiceModal
// ============================================================================

export function openBrowserSelfServiceModal(initialTab = 'videos') {
  ...
  silentVariableUpdate('ext_last_action', 'browser_ss_opened');
  ...
  function switchTab(tabName) {
    activeTab = tabName;
    currentVideoId = null;
    silentVariableUpdate('ext_current_tab', tabName);

    // Update tab buttons
    tabBar.querySelectorAll('.lfhbss-tab').forEach((btn) => {
  ...
  tabBar.addEventListener('click', (e) => {
    const tab = e.target.closest('.lfhbss-tab');
  ...
  function renderVideosTab() {
    content.innerHTML = `
      ...
      <div class="lfhbss-player" id="lfhbss-player">
        <button class="lfhbss-player-back" id="lfhbss-player-back">${BACK_ARROW} Back to all videos</button>
    ...
    // Video card clicks
    content.querySelectorAll('.lfhbss-video-card').forEach((card) => {
      card.addEventListener('click', () => showVideoPlayer(card.dataset.videoId));
    });
  ...
  function showVideoPlayer(videoId) {
    ...
    silentVariableUpdate('ext_video_played', videoId);
    ...
    const player = content.querySelector('#lfhbss-player');
    ...
    // Related videos
    const related = VIDEOS.filter((v) => v.id !== videoId).slice(0, 3);
    ...
    content.querySelectorAll('.lfhbss-related-card').forEach((card) => {
      card.addEventListener('click', () => showVideoPlayer(card.dataset.videoId));
    });
  }
  ...
  function renderFaqTab() {
    ...
    // Accordion behavior
    content.querySelectorAll('.lfhbss-faq-question').forEach((btn) => {
      btn.addEventListener('click', () => {
        const item = btn.closest('.lfhbss-faq-item');
        const wasExpanded = item.classList.contains('expanded');
        const answer = item.querySelector('.lfhbss-faq-answer');

        // Close all
        content.querySelectorAll('.lfhbss-faq-item.expanded').forEach((other) => {
  ...
  function renderResourcesTab() {
    content.innerHTML = `
      ...
      <div class="lfhbss-resource-grid">
        ${RESOURCES.map((r) => {
          if (r.type === 'action') {
            return `
            <button class="lfhbss-resource-card lfhbss-resource-action" data-handler="${r.handler}">
              <div class="lfhbss-resource-icon">${RESOURCE_ICONS[r.icon]}</div>
    ...
    // Attach action handlers for cross-nav buttons
    content.querySelectorAll('.lfhbss-resource-action').forEach((btn) => {
      btn.addEventListener('click', () => {
  ...
  applyResponsiveModal();
  window.addEventListener('resize', applyResponsiveModal);

  // Clean up resize listener when modal is removed
  const observer = new MutationObserver(() => {
    if (!document.getElementById('lfh-bss-modal')) {
      window.removeEventListener('resize', applyResponsiveModal);
      observer.disconnect();
    }
  });
